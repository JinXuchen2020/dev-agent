...
Then('工作流列表渲染', async ({ page }) => {
  ...
  await expect(page.getByText('Integration Fixture Workflow', { exact: true })).toBeVisible();
});

Then('提示请输入工作流名称', async ({ page }) => {
  await expect(page.getByText('请输入工作流名称', { exact: true })).toBeVisible({ timeout: 10000 });
});
